...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.effective_chat.type not in ("group", "supergroup"):
        await update.message.reply_text("Please add me to a group with you and the client, then run /register here.")
        return
    ...
    registered_groups[chat_id] = {"master_id": user_id, "state": None, "question_idx": None}
...
def schedule_questions(chat_id, master_id):
    ...
    for idx, q in enumerate(QUESTIONS):
        scheduler.add_job(
            ask_question,
            CronTrigger(
                hour=q["time"]["hour"], minute=q["time"]["minute"], timezone=IST
            ),
...
async def ask_question(chat_id, master_id, question_index):
    ...
    try:
        ...
        await application_instance.bot.send_message(chat_id=chat_id, text=question["text"], reply_markup=reply_markup)
...
async def answer_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in registered_groups:
        await query.edit_message_text(text="This group is not registered. Please run /register in this group.")
        return
    ...
    if ans == "yes":
        ...
        await context.bot.send_message(
            chat_id=master_id,
            text=f"⚠️ Deviation alert from group {chat_id}:\nQuestion: {question['text']}\nClient answered NO.",
        )
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "awaiting_photo":
        ...
        if update.message.text:
            ...
            await context.bot.send_message(
                chat_id=master_id,
                text=f"⚠️ Explanation from group {chat_id}:\nQuestion: {question['text']}\nExplanation: {explanation}",
            )
            registered_groups[chat_id]["state"] = None
            registered_groups[chat_id]["question_idx"] = None
        else:
